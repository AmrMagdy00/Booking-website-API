...
export class DestinationsController {
    ...
    create(@Body() dto:CreateDestinationDto,@UploadedFiles() files:Express.Multer.File[]){
        return this.destinationsService.Create(dto,files)
    }

    @Patch(':id')
    @UseInterceptors(FilesInterceptor('images'))
    update(@Param('id') id:string,@Body() dto:UpdateDestinationDto,@UploadedFiles() files:Express.Multer.File[]){
        this.destinationsService.update(id,dto,files)
    }
